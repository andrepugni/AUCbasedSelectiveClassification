...
def read_datasets_selnet(file, atts_dict, data_fold='data/'):
    """
...
def cross_entropy_selection2(y_true, hg, theta=.5, lamda=32, c=0.9):
    """
    ...
    """
    loss = torch.nn.functional.binary_cross_entropy(hg[:,1], y_true, weight=hg[:,-1])
    loss += lamda*(max(0,c-torch.mean(hg[:,-1])))**2
...
def get_predictions(model, device, valid_dl, test_dl, q):
    ...
    calibrated_theta = np.quantile(select_hold, 1-q,method='nearest')
...
def experiment(file_name, file_model_params='default', meta='selnet', classifier_string = 'resnet', 
    ...
    if file_name in ['adultNM', 'adultNMNOH', 'UCI_credit']:
        n_batch = 32
    elif file_name in ['lendingNOH','lending']:
        n_batch = 512
    else:
        n_batch = 128
    ...
    for cov in tqdm(coverages):
        ...
        if boot_iter ==0:
            ...
            for b in range(boot_iter+1):
                ...
                tmp['time_to_fit'] = time_to_fit
                results = pd.concat([results, tmp], axis=0)
                results.to_csv("results/results_{}_selnet_{}.csv".format(file_name, boot_iter))
...
if __name__=='__main__':
    ...
    parser.add_argument('--file', nargs="+", required=False, default=['UCI_credit' ,'GiveMe','CSDS1', 'CSDS2', 'CSDS3','adultNM', 'lending'])
    ...
    for file in tqdm(filelist):
        ...
        tmp.to_csv('results/AUCROSSPAPER_{}_{}_{}epochs_lamda{}_td{}.csv'.format(classifier_string, file, max_epochs, lamda, td))
        res = pd.concat([res, tmp], 0)
        res.to_csv('results/AUCROSSPAPER_{}_{}_{}epochs_lamda{}_td{}.csv'.format(classifier_string, file, max_epochs, lamda, td))
