...
# -*- coding: utf-8 -*-
"""
Code for experiments using AUCross and Plug-In
"""
import pandas as pd
import numpy as np
...
from cmeta import AUCross, PlugInRule, PlugInRuleAUC, SCross
...
def get_results(file, atts_dict,file_params='default.csv', 
                metas=['scross', 'cross', 'cross2', 'scross2','pluginAUC', 'pluginAUC__whole', 'plugin'],
                classifier_string='lgbm', max_ep = 300,
                boot_iter=1000, default = True, given=True, cv=5, scaler_dict ={'CSDS1': False, 'CSDS2': False,
                                                                                'CSDS3': False, 'GiveMe': True,
                                                                                'adultNMNOH': True, 'adultNM': True,
                                                                                'UCI_credit': True,'lending': True,
                                                                                'lendingNOH': True},
                batch_dict = {'adultNM': 32, 'adultNMNOH': 32, 'UCI_credit': 32,
                              'lendingNOH': 512,'lending': 512, 'CSDS1': 128,
                              'CSDS2': 128,'CSDS3': 128,'GiveMe': 128}):
    """
    ...
    metas : list, optional
        List of selective classifiers. The default is ['cross','plugin'].
    classifier_string : str, optional
        The name of base model to use. Possible choices are:
           ...
           - 'resnet' for ResNet implementation.
        The default is 'lgbm'.
    max_ep : int, optional
        The number of epochs used to train neural networks. The default is 300.
    ...
    given : boolean, optional
        It is used as in the main paper to consider the classifier as given.
        If False, for AUCross it uses an ensemble strategy to get final scores
        by averaging folds scores
        The default is True.
    cv : int, optional
        The number of folds. The default is 5.
    scaler_dict: dict, optional
        The dictionary that specifies over which dataset the scaling is performed.
    batch_dict : dict, optional
        The dictionary that specifies the batch size for each dataset.
    ...
    for meta in metas:
        ...
        if classifier_string=='lgbm':
            ...
            clf_base = lgb.LGBMClassifier(random_state=42, **dict_params)
        elif (classifier_string=='resnet')&(meta not in ['selnet','sat','dg']):
            ...
            if file in ['CSDS1', 'lending', 'GiveMe']:
                clf = SelfAdaptiveClassifier('{}2'.format(classifier_string), max_epochs=max_ep, td=True, scaler_bool=scaler_dict[file], n_batch=batch_dict[file])
            else:
                clf = SelfAdaptiveClassifier(classifier_string, max_epochs=max_ep, td=True, scaler_bool=scaler_dict[file], n_batch=batch_dict[file])
            print(clf.max_epochs) 
        elif (meta=='dg')&(classifier_string=='resnet'):
            if file in ['CSDS1', 'lending', 'GiveMe']:
                clf = SelfAdaptiveClassifier('{}2'.format(classifier_string), max_epochs=max_ep, td=True, scaler_bool=scaler_dict[file], n_batch=batch_dict[file], crit='dg')
            else:
                clf = SelfAdaptiveClassifier(classifier_string, max_epochs=max_ep, td=True, scaler_bool=scaler_dict[file], n_batch=batch_dict[file], crit='dg')
            print(clf.max_epochs) 
        ...
        print(cv)    
        if meta=='cross':
            clf = AUCross(clf_base, quantiles=[0.01,0.05,0.1,0.15,0.2,0.25], cv=cv)
        elif meta=='scross':
            ...
            clf = PlugInRuleAUC(clf_base, quantiles=[0.01,0.05,0.1,0.15,0.2,0.25], seed=42)

        start_time = time()
        clf.fit(X_train, y_train)
        end_time = time()
        time_to_fit = (end_time - start_time)
        if (given==True)&((meta in ['pluginAUC_whole'])):
            print(meta)
            start_time = time()
            clf_base.fit(X_train, y_train)
            end_time = time()
            time_to_fit+=(end_time - start_time)
            scores = clf_base.predict_proba(X_test)[:,1]
            preds = clf_base.predict(X_test)
...
if __name__=='__main__':
    ...
    parser.add_argument('--filelist', nargs='+', default = [ 'adultNM', 'UCI_credit', 'GiveMe', 'CSDS1', 'CSDS2', 'CSDS3', 'lending'])
    parser.add_argument('--metas', nargs='+', default = ['cross','plugin', 'pluginAUC','scross', 'pluginAUC_whole'])
    ...
    if n_jobs==1:
        for file in tqdm(filelist):
            ...
            results = pd.concat([results, tmp], axis=0)
            class_file = 'AUCROSSPAPER_results_real_data_{}_{}_{}_{}_{}_{}_{}.csv'.format(classifier_string, boot_iter, default, given, cv, metas, filelist)
